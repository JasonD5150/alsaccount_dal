 ...
 */
public class ${declarationName}DAO extends ${pojo.importType("fwp.als.apptrack.hibernate.BaseHibernateDAO")} {

    private static final ${pojo.importType("org.slf4j.Logger")} log = ${pojo.importType("org.slf4j.LoggerFactory")}.getLogger(${pojo.getDeclarationName()}DAO.class);
...
<#if clazz.identifierProperty?has_content>
	...
	public List<${declarationName}> findAll() {
		...
		try {
            ...
            ${pojo.importType("org.hibernate.Query")} queryObject = getSession().createQuery(queryString);
			return (List<${declarationName}>)queryObject.list();
		} catch (RuntimeException re) {
